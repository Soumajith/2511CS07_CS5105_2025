...
public:
    int timer;
    void dfs(int u,int p, vector<vector<int>>&adj, vector<int>&tin, vector<int>&low, vector<int>&vis, vector<pair<int,int>>&bridges){
    ...
    void solve() {
        ...
        for(int i=0;i<n;i++) if(!vis[i]) dfs(i,-1,adj,tin,low,vis,bridges);
